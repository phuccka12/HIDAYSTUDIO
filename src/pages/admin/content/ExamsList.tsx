...
/* ---------------- Page: Exams List (UI polish, same logic) ---------------- */
export default function ExamsListPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res: any = await adminListExams({ page, limit: 20 });
      setItems(res.items || []);
    } finally {
      setLoading(false);
    }
  ...
  async function handleUpdate(id: string, payload: any) {
    if (payload && payload._id) {
      setItems((prev) => prev.map((p) => (String(p._id) === String(payload._id) ? payload : p)));
      setEditing(null);
      return;
    }
    await adminUpdateExam(id, payload);
    setEditing(null);
    load();
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-4">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Đề thi</h2>
          <p className="mt-1 text-sm text-gray-500">Quản lý danh sách đề thi, tạo mới, chỉnh sửa và xuất bản.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50">
              <tr className="text-sm text-gray-600">
                ...
                <th className="p-3 font-semibold">Phiên bản</th>
              ...
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="p-3"><div className="h-4 w-12 animate-pulse rounded bg-gray-200" /></td>
                    <td className="p-3 text-right"><div className="ml-auto h-8 w-40 animate-pulse rounded bg-gray-200" /></td>
                  </tr>
                ))
              ) : items.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-10 text-center text-sm text-gray-600">
                    Chưa có đề thi nào. Nhấn <span className="font-medium">Tạo đề thi</span> để bắt đầu.
                  </td>
                </tr>
              ) : (
                items.map((it) => (
                  <tr key={it._id} className="border-t hover:bg-gray-50/60">
                    ...
                    <td className="p-3 align-middle">
                      <span className="rounded-lg bg-gray-50 px-2 py-1 text-xs text-gray-700 ring-1 ring-inset ring-gray-200">
                        {it.version || 0}
                      </span>
                    </td>
                    <td className="p-3 align-middle">
        ...
        <div className="divide-y md:hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-sm text-gray-600">Chưa có đề thi nào.</div>
          ) : (
            items.map((it) => (
      ...
      {showCreate && (
        <Modal title="Tạo đề thi mới" onClose={() => setShowCreate(false)}>
          <ExamForm onSave={handleCreate} onCancel={() => setShowCreate(false)} />
        </Modal>
      )}

      {/* Edit Modal */}
      {editing && (
        <Modal title="Chỉnh sửa đề thi" onClose={() => setEditing(null)}>
          <ExamForm initial={editing} onSave={(p) => handleUpdate(editing._id, p)} onCancel={() => setEditing(null)} />
        </Modal>
      )}
    </div>
  );
